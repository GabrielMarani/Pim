...
const api = {
    ...
    async deletarProduto(id) {
        ...
        if (!resp.ok && resp.status !== 204) {
            const err = await resp.json().catch(() => ({ mensagem: 'Erro ao deletar produto.' }));
    ...
    async cancelarPedido(id) {
        ...
        if (!resp.ok && resp.status !== 204) {
            const err = await resp.json().catch(() => ({ mensagem: 'Erro ao cancelar pedido.' }));
            throw new Error(err.mensagem || `HTTP ${resp.status}`);
        }
        return true;
    },
...
async function fetchJson(url, options = {}) {
    ...
    try {
        ...
        if (err.name === 'AbortError') {
            throw new Error('Tempo limite excedido. O servidor está demorando para responder — tente novamente.');
        }
